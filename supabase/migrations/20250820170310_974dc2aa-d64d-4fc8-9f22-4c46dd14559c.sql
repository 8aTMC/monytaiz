...
ADD COLUMN IF NOT EXISTS read_by_recipient boolean DEFAULT false;

-- Add index for performance
CREATE INDEX IF NOT EXISTS idx_messages_read_status 
ON messages (conversation_id, read_by_recipient, created_at);

-- Update conversations to track unread count
ALTER TABLE conversations 
ADD COLUMN IF NOT EXISTS unread_count integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS latest_message_content text,
ADD COLUMN IF NOT EXISTS latest_message_sender_id uuid;
