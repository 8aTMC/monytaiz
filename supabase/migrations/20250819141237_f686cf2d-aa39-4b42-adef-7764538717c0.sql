-- Fix the deletion functions to actually delete from auth.users as well
...
BEGIN
  ...
  -- Add audit log entry BEFORE deleting from auth
  ...
  -- IMPORTANT: Also delete from auth.users table completely
  -- Note: This requires admin client with service role key in edge function
  -- Will be handled by separate cleanup function due to RLS restrictions
...
-- Update the expired users cleanup function
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'message', format('Permanently deleted %s users - auth cleanup needed', deletion_count),
    'auth_cleanup_needed', true,
