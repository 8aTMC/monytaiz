-- Update the immediately_delete_user function to include chat data deletion
...
BEGIN
  ...
  -- Delete all chat messages and media sent by the user
  -- Note: Update table names based on your actual chat schema
  ...
  -- Delete chat media files
  DELETE FROM chat_media WHERE sender_id = target_user_id;
  DELETE FROM message_attachments WHERE user_id = target_user_id;
  
  -- Delete chat rooms/conversations where user was participant
  DELETE FROM chat_participants WHERE user_id = target_user_id;
  DELETE FROM conversation_members WHERE user_id = target_user_id;
...
-- Update the permanently_delete_expired_users function to include chat data deletion
...
BEGIN
  ...
  LOOP
    ...
    DELETE FROM direct_messages WHERE sender_id = user_id OR recipient_id = user_id;
    
    -- Delete chat media files
    DELETE FROM chat_media WHERE sender_id = user_id;
    DELETE FROM message_attachments WHERE user_id = user_id;
    
    -- Delete chat rooms/conversations where user was participant
    DELETE FROM chat_participants WHERE user_id = user_id;
    DELETE FROM conversation_members WHERE user_id = user_id;
