-- Fix get_user_conversations function to handle admin users correctly
...
AS $function$
  ...
  WHERE c.is_active = true
    ...
    AND (
      -- Normal user access
      (is_creator_param AND c.creator_id = user_id) OR 
      (NOT is_creator_param AND c.fan_id = user_id) OR
      -- Admin override - can see all conversations
      EXISTS (
