-- Create welcome message system and chat deletion/restoration logic

-- First, let's add a status field to conversations for deletion tracking
...
ALTER TABLE conversations ADD COLUMN IF NOT EXISTS deletion_scheduled_for TIMESTAMP WITH TIME ZONE;

-- Add similar status to messages
ALTER TABLE messages ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active';
ALTER TABLE messages ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE;

-- Create function to send welcome message when fan completes signup
...
-- Create trigger for welcome messages
DROP TRIGGER IF EXISTS send_welcome_message_trigger ON profiles;
CREATE TRIGGER send_welcome_message_trigger
  AFTER UPDATE ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION send_welcome_message_to_fan();

-- Function to handle conversation deletion when user is deleted
...
-- Create trigger for conversation deletion/restoration
DROP TRIGGER IF EXISTS handle_conversation_user_deletion_trigger ON profiles;
CREATE TRIGGER handle_conversation_user_deletion_trigger
  AFTER UPDATE ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION handle_conversation_user_deletion();
